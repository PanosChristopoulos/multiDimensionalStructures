...
for i in range(len(articlesDataframe)): 
    try:
        ...
        shingle_title = list(shingle(articlesDataframe['title'][i],3)) #Call function shingle
        final_set.append(shingle_title)
...
def createHash(vector:list):
    '''
    use this func to create our signatures
    '''
    signature = []
...
shingleSignature = list(createHash(shingleOne))# Create Signature
